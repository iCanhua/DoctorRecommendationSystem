...
public class PreDiagnosisImp implements PreDiagnosis {
	...
	public InterQuestion prediagnosis(Patient patient)
		...
		for (Symptom element : patient.getHasSymptoms()) {
			interDisease.addAll(ontParserDao
					.getRelativeDiseaseBySymptom(element));
		}
		// 初始化可能患有的疾病,并赋予初始值。
		...
		for (Disease disease : interDisease) {
			interSymptoms.addAll(ontParserDao.getRelativeSymptomByDisease(
					"是表现", disease));
		}
		...
		for (Disease disease : interDisease) {
			interPathogeny.addAll(ontParserDao.getRelativePathogenyByDisease(
					"导致", disease));
		}
		...
		for (Disease disease : interDisease) {
			interMedicalHistory.addAll(ontParserDao
					.getRelativeDiseaseByDisease("引发", disease));
		}
	...
	public Patient prediagnosis(Patient patient, InterQuestion interQuestion) {
		...
		patient.getHasMedicalHistory().addAll(
				interQuestion.getHasMedicalHistory());
		return patient;
	}
