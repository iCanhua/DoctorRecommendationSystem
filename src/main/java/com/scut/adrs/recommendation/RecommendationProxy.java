...
import com.scut.adrs.recommendation.service.imp.DiagnosisImp;
...
//		PreDiagnosis pd=ctx.getBean(PreDiagnosisImp.class);
		...
		patient.addSymptom(ontDaoUtils.getNS()+"心悸");
...
//		diagnose.diagnose(patient);
		...
		proxy.prediagnosis(patient, question);
		...
		for(Doctor dt:b){
			...
			Individual  ind=model.getResource(dt.getName()).as(Individual.class);
	...
	public Map<Doctor, Float> doctorMatch(Patient patient) {
		...
		if(!patient.isPreDiagnosis()){
			throw new DiagnoseException("please prediagnose the patient before you match a doctor !");
		}
