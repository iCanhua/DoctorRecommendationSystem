...
public class DescriptionComprehension implements DescriptionParser,Match{
	...
	public InterConceptQuestion comprehend(String description){
		...
		for(Resource resource:reasourceList){
			if("症状".equals(resource.getDomainType())) interConceptQuestion.getSymptoms().add((Symptom)resource);
			if("体征".equals(resource.getDomainType())) interConceptQuestion.getBodySigns().add((BodySigns)resource);
			if("病因".equals(resource.getDomainType())) interConceptQuestion.getPathogeny().add((Pathogeny)resource);
			if("疾病及综合症".equals(resource.getDomainType())) interConceptQuestion.getMedicalHistory().add((Disease)resource);
		}
		
		reasourceList=matcher.approximateMatch(result);
		for(Resource resource:reasourceList){
			if("症状".equals(resource.getDomainType())) interConceptQuestion.getHasSymptoms().add((Symptom)resource);
			if("体征".equals(resource.getDomainType())) interConceptQuestion.getHasBodySigns().add((BodySigns)resource);
			if("病因".equals(resource.getDomainType())) interConceptQuestion.getHasPathogeny().add((Pathogeny)resource);
			if("疾病及综合症".equals(resource.getDomainType())) interConceptQuestion.getHasMedicalHistory().add((Disease)resource);
		}
	...
	public static void main(String[] args) {
		...
		String description="我槽，你呢，这是我心绞痛第心尖冲动一亚急性渗液缩窄性心包炎次心肌病短剧哦。心悸";
		Result result=parser.parse(description);
