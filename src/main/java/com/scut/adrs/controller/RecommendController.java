...
public class RecommendController {
	...
	public String x() {
		...
		String NS = ontDaoUtils.getNS();
		set.add(new Symptom(NS + "关节痛"));
		set.add(new Symptom(NS + "手足麻木"));
		set.add(new Symptom(NS + "抽搐"));
		set.add(new Symptom(NS + "肢体沉重"));
		Patient patient = new Patient();
		patient.setHasSymptoms(set);
		try {
			InterQuestion question = proxy.prediagnosis(patient);
			System.out.println("症状" + question.getHasSymptoms().size());
			System.out.println("病因" + question.getHasPathogeny().size());
			System.out.println("病史" + question.getHasMedicalHistory().size());
		} catch (UnExistRdfException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
